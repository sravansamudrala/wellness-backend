...
Status: implemented and deployed (`main`, PR #40) — no open implementation questions remain (§24). A post-deploy incident and its fix are documented in Revision note 5 below.
...
**Revision note 5 (post-deploy incident — dropped Settings fields and imports):** PR #40 squash-merged this feature into `main`, but the feature branch's own sync-with-`main` commit (reconciling this feature against unrelated, concurrently-developed `aiwt` work) had a botched conflict resolution: `main` and this feature both added new content in the same regions of `app/core/config.py` and `app/services/push_service.py`, and the resolution kept only one side in each spot. Effect in production: every authenticated call to `GET /api/v1/electricity/insights` returned `500 AttributeError: 'Settings' object has no attribute 'meter_slab_min_evaluation_days'`, since `evaluate_switch_recommendation` (already deployed) referenced `Settings` fields that never actually landed on `main`. `PushService.dispatch_meter_slab_recommendation` would have failed identically with `NameError: name 'local_today' is not defined` on its next scheduled run, for the same reason. No business logic, acceptance criterion, or confirmed decision in this spec changed or was found incorrect — every other file PR #40 touched was diffed against the pre-incident feature-branch commit and found byte-identical; the damage was isolated to the four `meter_slab_*` `Settings` fields and four import lines in `push_service.py` (`local_today`, `Meter`/`MeterShare`, `FeatureFlag`, `evaluate_switch_recommendation`), both restored alongside the unrelated `aiwt` content rather than replacing it, on branch `fix/restore-meter-slab-settings-after-merge`. **Lesson for future merges into a file under concurrent development from two branches: a clean (no-conflict-marker) merge is not proof the result is correct — diff the merged file against each side's last known-good state, not just against the immediately-preceding commit.**
