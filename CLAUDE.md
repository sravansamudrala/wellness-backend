...
`DATABASE_URL` must be set in `.env` (loaded by [app/core/config.py](app/core/config.py) via pydantic-settings) — it points at a Supabase PostgreSQL instance via the connection pooler (psycopg 3 driver). The engine sets `pool_pre_ping=True` so connections dropped during Render idle / by the pooler reconnect transparently. Other `.env` settings: `SQL_ECHO` (bool, default off), `LOG_LEVEL` (default `INFO`, see **Logging** below), `AUTH_RATE_LIMIT` (default `10/minute`, see **Authentication** below), the Web Push vars below (`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`, `DISPATCH_TOKEN`, `REMINDER_TIMEZONE`), `GROQ_API_KEY`/`GROQ_MODEL` (default model `qwen/qwen3.8-27b`, since Groq deprecated `llama-3.1-8b-instant` on 2026-08-16) for the AI-generated skincare/water messages — see **Skincare endpoints** and **Water endpoints** below (unset `GROQ_API_KEY` just falls back to the old rule-based messages, no functionality is lost), `AIWT_SERVICE_URL` for the standalone Aiwt water-message model service (see **Aiwt** below; unset falls back to the static `WATER_MESSAGE`), and the password-reset vars (`SENDGRID_API_KEY`, `MAIL_FROM`, `FRONTEND_URL`, `PASSWORD_RESET_RATE_LIMIT` default `3/hour`) — see **Authentication & per-user data** below. `ENABLE_API_DOCS` (bool, default `true`) hides `/docs`/`/redoc` when set `false` in Render's production env. There is no linter config or Makefile in the repo yet.
...
- **Primitives** — [app/core/security.py](app/core/security.py): `hash_password`/`verify_password` (bcrypt), `create_access_token(user_id, token_version)`/`decode_token` (PyJWT, HS256, `sub`=user id, `ver`=token version, `exp`). Secret is `settings.jwt_secret` (**required** env `JWT_SECRET` — app won't boot without it; also set on Render). Deps: `PyJWT`, `bcrypt` in requirements. `decode_token(token) -> Optional[str]` (returns just `sub`) keeps its original contract unchanged since `tests/test_skincare.py` depends on it directly; `decode_token_claims(token) -> Optional[dict]` (full payload) was added alongside it for `get_current_user` instead of changing `decode_token`'s return type. Both delegate to a shared private `_decode_payload`.
- **User model** — [app/models/user.py](app/models/user.py) (`users`: id, email unique, hashed_password, `token_version` int default `0`, created_at).
- **Auth endpoints** — [app/api/auth.py](app/api/auth.py), prefix `/api/v1/auth`: `POST /register`, `POST /login` (both return `{access_token, token_type}`), `POST /forgot-password`, `POST /reset-password`, `GET /me`. `AuthService` in [app/services/auth_service.py](app/services/auth_service.py) (register/authenticate/password-reset — no legacy-claim logic in the hot path).
- **The auth gate** — [app/api/deps.py](app/api/deps.py) `get_current_user` (FastAPI `Depends` + `HTTPBearer`): decodes the `Authorization: Bearer <token>` header via `decode_token_claims` → returns `user_id: UUID`, else 401. It also compares the token's `ver` claim against the user's current `token_version` in the DB (one lookup per request) and 401s on mismatch — see **Session invalidation on password reset** below. **Every** protected endpoint adds `user_id: UUID = Depends(get_current_user)` and passes it to its service; services filter every query by `user_id`. The catalog reads are login-gated too but **not** user-scoped (shared) — they use `_user_id: UUID = Depends(get_current_user)` purely as a gate.
- **Per-user tables** (have `user_id` FK): `skincare_entries` (unique swapped `date` → `(user_id, date)`), `reminder_settings` (unique `user_id`), `push_subscriptions`, `reminder_dispatch_log` (unique `(user_id, sent_on, slot)`), `gym_state` (unique `user_id`), `workout_sessions`, `password_reset_tokens` (FK `user_id`, no uniqueness constraint on it — a user can have multiple outstanding tokens). Children (`session_exercises`/`session_sets`) inherit ownership via their parent FK. Migration: `migrations/versions/2cae70d68811_*`.
...
- **Rate limiting** ([app/core/rate_limit.py](app/core/rate_limit.py), `slowapi`): `/register`, `/login`, and `/reset-password` are limited to `settings.auth_rate_limit` (env `AUTH_RATE_LIMIT`, default `10/minute`) per client IP; `/forgot-password` gets its own stricter `settings.password_reset_rate_limit` (env `PASSWORD_RESET_RATE_LIMIT`, default `3/hour`) since it can be used to spam a stranger's inbox. Disabled in tests (`tests/conftest.py` sets `limiter.enabled = False`) since `TestClient` requests all share one fake address. Verified in production that Render/Cloudflare forward the real client IP correctly (`request.client.host` isn't a shared proxy address).
- **Password reset** — `POST /forgot-password` (`AuthService.request_password_reset`) always returns the same generic message regardless of whether the email exists, and issues a single-use `PasswordResetToken` ([app/models/password_reset_token.py](app/models/password_reset_token.py), expires after `settings.password_reset_token_expire_minutes` = 45 min) — only a SHA-256 hash of the raw token is stored, the raw value only ever exists in the emailed link. Email is sent via [app/services/email_service.py](app/services/email_service.py) using **SendGrid's HTTP API**, never SMTP — **Render blocks all outbound SMTP** (confirmed empirically: `SMTP_SSL`/465 fails immediately with `Network is unreachable`, STARTTLS/587 times out even with IPv4 forced — a firewall silently dropping the connection). `mail_from` must be a verified SendGrid Single Sender. The reset link points at `settings.frontend_url` (env `FRONTEND_URL`). `POST /reset-password` (`AuthService.reset_password`) validates the token, updates the password, and **invalidates every previously-issued JWT** by incrementing `User.token_version` — see below.
- **Session invalidation on password reset** — a stolen/leaked JWT is invalidated the moment its owner resets their password: `AuthService.reset_password` increments `User.token_version`, and `get_current_user` rejects any token whose `ver` claim no longer matches (a missing `ver` claim, from pre-feature tokens, defaults to `0` so old sessions survived the deploy). **Deliberately no caching** in front of that DB check — an earlier in-memory-cache design (to avoid a DB query on every authenticated request) had a real lost-update race where a stale in-flight read could un-invalidate the exact token being reset; the plain per-request DB check was chosen instead as simpler and always correct (cheap here since Render and Supabase both sit in the same region). If extending this (e.g. a "log out everywhere" button), reuse `token_version` rather than building a parallel revocation mechanism.
- **Email enumeration is intentionally inconsistent across endpoints** — `/login` and `/forgot-password` deliberately stay generic (never reveal whether an email exists), but `POST /register` returns `"That email is already registered."` on a duplicate, which does let anyone enumerate registered emails. This was reviewed pre-rollout and left as-is (same tradeoff most mainstream apps make; only discloses account existence, not credentials) — don't flag it as a bug or re-propose fixing it unless the threat model materially changes.
...
### Aiwt — fine-tuned water-message model (now a separate service, not in-process)

**Migration complete (2026-08-13).** Aiwt (the fine-tuned flan-t5-small that generates personalized water-reminder text) used to run in-process here, quantized to INT8 specifically to fit Render's shared 512MB RAM budget — but INT8 quantization turned out not to be numerically portable across CPU architectures, which caused a real production bug (garbled/hallucinated push notifications, confirmed via a byte-for-byte cross-platform test showing 0/80 identical outputs between Mac ARM and Render x86_64). Fixing that properly (dropping quantization) needed more RAM than fits alongside the rest of this app, so Aiwt now lives in its own standalone repo/service, **`wellness-aiwt`** (`github.com/sravansamudrala/wellness-aiwt`), deployed separately on Render. `app/ai/water_message/` (the old in-process code) has been deleted from this repo entirely — the full investigation history (root cause, every memory-optimization attempt, the vocabulary-pruning fix that finally solved it) lives in that repo's `CLAUDE.md`, not here.

- **Integration:** [app/services/aiwt_service.py](app/services/aiwt_service.py) `generate_water_message(amount_ml, goal_ml, current_streak, hour)` — a plain `requests.post` to `{AIWT_SERVICE_URL}/v1/generate`, same try/except-and-fallback pattern as `ai_message_service.py`'s Groq calls (returns `None` on any failure, unreachable service, or unset URL — caller falls back to the static message). Called from `PushService.dispatch_water_due` when `settings.water_message_model_enabled`.
- **Config:** `AIWT_SERVICE_URL` env var (empty = feature disabled, same effect as `water_message_model_enabled=false`). Bucketing (raw amount/goal/streak/hour → the model's actual input format) now happens on the `wellness-aiwt` side (`contexts/water.py` there), not duplicated here — this repo just sends raw values.
- **Known gap:** `wellness-aiwt`'s `/v1/generate` is currently a fully public, unauthenticated endpoint (no equivalent of `DISPATCH_TOKEN` yet) — tracked as a follow-up in that repo, not blocking this integration.
